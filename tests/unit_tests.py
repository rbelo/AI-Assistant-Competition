...
from streamlit.modules import negotiations, schedule
from streamlit.modules import student_playground as playground
from streamlit.modules.drive_file_manager import get_text_from_file
...
def create_test_deal(initial_value, rounds, success):
    ...
    return {"value": initial_value if success else 0, "rounds": rounds, "success": success}
...
def create_test_tables():
    ...
    for i in range(10):
        ...
        metrics.record_prompt_metrics(user_id=user_id, prompt_text=prompt_text, response_time=random.randint(1, 5))
...
def test_google_drive_connection():
    ...
    try:
        ...
        # Test authentication
        service_account.Credentials.from_service_account_info(drive_creds)
...
def test_authentication():
    ...
    assert st.session_state["instructor"]
    assert st.session_state["user_id"] == "test_user"

    # Test invalid login
    st.session_state["authenticated"] = False
    assert not st.session_state["authenticated"]
...
def test_metrics_accuracy():
    ...
    game_id = "test_game_1"
    metrics.record_game_interaction(
        user_id=user_id, game_type="test_game", game_id=game_id, completion_time=300, score=85
    )
...
def test_error_handling():
    ...
    if not use_real_drive:
        ...
        assert get_text_from_file("non_existent_file.txt") is None

    # Test metrics recording with invalid data
    assert metrics.record_page_exit(None, -1) is False


def test_game_logic():
    """Test game logic and scoring"""
    # Test game scoring
    score = calculate_game_score(completion_time=300, correct_answers=8, total_questions=10)
    assert 0 <= score <= 100, "Score should be between 0 and 100"

    # Test negotiation logic
    deal = create_test_deal(initial_value=10000, rounds=5, success=True)
...
def test_negotiation_logic():
    ...
    round_result = negotiations.process_negotiation_round(game_id=game_id, player_offer=10000, ai_offer=12000)
    assert round_result is not None
    assert "status" in round_result
    assert "next_round" in round_result

    # Test negotiation completion
    final_deal = negotiations.complete_negotiation(game_id=game_id, final_value=11000)
