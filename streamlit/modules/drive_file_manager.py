...
def upload_text_as_file(text_content, filename):
    ...
    file_metadata = {"name": f"{filename}.txt", "parents": [parent_folder_id] if parent_folder_id else []}
...
def find_file_by_name(filename):
    ...
    results = service.files().list(q=q, spaces="drive", fields="files(id, name)", pageSize=10).execute()
...
def find_file_by_name_without_timestamp(base_filename):
    ...
    query = f"name contains '{base_filename}' and trashed=false"
    if parent_folder_id:
